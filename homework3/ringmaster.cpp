...
int main(int argc, char *argv[]){
	...
	std::string *port=new std::string[Ringmaster.getplayernum()];
  ...
  int * player_fd = (int*)malloc(Ringmaster.getplayernum() * sizeof(*player_fd));
  int i=0;
 ...
 // std::cout<<*message<<std::endl;
  char buffer[512];
  sprintf(buffer, "%d", i);
  send(player_fd[i],buffer,30,0);
  //get the ready information from each player
  recv(player_fd[i],buffer,50,0);
